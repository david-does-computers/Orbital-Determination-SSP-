...
class GaussMethod:
    ...
    C = 173.144643267                        # Speed of light [AU/Day]
    ...
    def __init__(self, observations: tuple[Observation]) -> None:
        ...
        self.corrected_times = np.array([observations[0].get_time(), observations[1].get_time(), observations[2].get_time()])

        self._calc_tau_intervals()
    ...
    def iterative_honing(self, tolerance: np.float64=1e-10):
        ...
        :param tolerance: threshold which dictates the maximum change in position vector before quitting iteration
        """
        self.prev_r = 1.1 * self.r_vectors[1]
        for i in range(500) : #TODO: Increased efficiency using magnitude squared?
            self._calc_rho_vectors()
            self._light_time_corrections()
            self._calc_r_vectors()
            self._calc_f_g_values()
            self._calc_a_values()
            if np.abs(1 -np.linalg.norm(self.prev_r)/odlib.magnitude(self.r_vectors[1])) < tolerance:
                print("broken")
                break
    ...
    def _calc_D_values(self):
        ...
        for i in range(3):
            self.D_values[0, i] = np.array([
                ...
                self.observations[2].get_rho_hat().dot(np.cross(self.observations[i].get_sun_vector(), self.observations[1].get_rho_hat()))
            ])
    ...
    def _calc_tau_intervals(self):
        """
        Calclautes the tau intervals
        """  
        self.taus = np.array([                                                      #Look to Dubson OD Packet for formatting
            self.K*(self.corrected_times[2] - self.corrected_times[0]), 
            self.K*(self.corrected_times[0] - self.corrected_times[1]), 
            self.K*(self.corrected_times[2] - self.corrected_times[1])])

    def _light_time_corrections(self):
        """
        Accounts for the light travel time updating the actual observation times and the tau intervals
        """
        for i in range(3):
            self.corrected_times[i] = self.observations[i].get_time() - self.rhos[i] / self.C
